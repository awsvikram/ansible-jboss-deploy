...
export JAVA_HOME
JBOSS_CONF=/app/jboss/jboss-eap-7.0.4/configurations/{{ app_name }}.conf
...
chown $JBOSS_USER $(dirname $JBOSS_CONSOLE_LOG) || true
  chown $JBOSS_USER $JBOSS_CONSOLE_LOG || true
  ...
  if [ ! -z "$JBOSS_USER" ]; then
    currentUser=`id | awk '{ print $1 }' | sed -e 's/.*[(]//' | sed -e 's/[)].*//'`
